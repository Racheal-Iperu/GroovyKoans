...
class Koan11 extends GroovyTestCase {
    ...
    void test02_MoreOnInsertingData() {
        ...
        Sql.withInstance('jdbc:h2:mem:groovyDb2', 'sa', '', 'org.h2.Driver') { db ->
            ...
            // ------------ START EDITING HERE ----------------------
             //new AntBuilder().copy(file:"src/test/groovy/org/groovykoans/koan11/cast.txt" )
            ...
            // ------------ START EDITING HERE ----------------------
            def reader = new XmlSlurper().parse('src/test/groovy/org/groovykoans/koan11/cast2.xml')
            reader.character.@actor.each{it->
                db.execute('insert into PERSON (FIRSTNAME, LASTNAME) values (?, ?)',it.text().split())
            }
            ...
            // ------------ START EDITING HERE ----------------------
            person.add(FIRSTNAME:'Racheal',LASTNAME:'Iperu')
    ...
    void test03_IteratingResults() {
        Sql.withInstance('jdbc:h2:mem:groovyDb3', 'sa', '', 'org.h2.Driver') { db ->
            ...
            def lastNameRose
            // ------------ START EDITING HERE ----------------------
           def rose = db.firstRow('SELECT LASTNAME FROM PERSON WHERE FIRSTNAME= \'Rose\'')
            ...
            def eCount = 0
            // ------------ START EDITING HERE ----------------------
            def num = db.eachRow( 'SELECT LASTNAME FROM PERSON '){ it ->
    ...
    void test04_UpdatingTables() {
        Sql.withInstance('jdbc:h2:mem:groovyDb4', 'sa', '', 'org.h2.Driver') { db ->
            ...
            // ------------ START EDITING HERE ----------------------
            db.eachRow("SELECT * FROM PERSON p WHERE p.firstname like '%a%'"){it ->
                it.firstname = 'Alf'
